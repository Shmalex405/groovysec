...
export default function Resources() {
  ...
  const resources: Resource[] = [
    ...
    /* TEMP: Skills page unpublished 2026-07-16 — this research card funnels to /skills; restore to relist.
    {
      icon: ShieldAlert,
      tag: "RESEARCH",
      tagColor: "bg-[#B3261E]/10 text-[#B3261E] border-[#B3261E]/25",
    ...
    */
    {
      icon: Award,
      tag: "NEWS",
      tagColor: "bg-[#2E7D32]/10 text-[#2E7D32] border-[#2E7D32]/25",
              ...
              {resources.map((resource) => {
                ...
                return (
                  <StaggerItem key={resource.title}>
                    <article className="p-8 bg-white rounded-xl border border-[#0F1B2D]/10 shadow-[0_1px_2px_rgba(15,27,45,0.05),0_12px_32px_rgba(15,27,45,0.07)] hover:border-[#0F1B2D]/20 hover:shadow-[0_2px_4px_rgba(15,27,45,0.06),0_16px_40px_rgba(15,27,45,0.10)] transition-all duration-300">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="w-9 h-9 bg-[#0F1B2D]/[0.03] border border-[#0F1B2D]/10 rounded-lg flex items-center justify-center">
                          <Icon className="w-4 h-4 text-[#51617A]" />
                        </div>
                        <span className={`px-2 py-0.5 text-[10px] font-mono font-bold border rounded ${resource.tagColor}`}>
                          {resource.tag}
                        </span>
                      </div>
                      <h2 className="text-xl font-bold text-[#0F1B2D] mb-3">{resource.title}</h2>
