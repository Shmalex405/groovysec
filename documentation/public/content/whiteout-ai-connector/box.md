...
5. Initial scan begins from the root folder (id=`0`) and traverses
...
- **Initial scan duration:** depends on folder tree depth + count.
  ~3s per doc plus a small per-folder enumeration overhead.
...
- **Slow initial scan** — Box's `/folders/{id}/items` endpoint
  paginates 100 items at a time. Tens-of-thousand-file accounts can
  take an hour-plus. Subsequent ticks are fast (events-based).
